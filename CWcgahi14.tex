...
\begin{figure}[!h]
		...
		\label{fig:framework}
\end{figure}
Fig.~\ref{fig:framework} shows a complete view of our proposed system.
...
\begin{algorithm}[h]
  ...
  \begin{algorithmic}[1]
    ...
    \State $Projection = Data\bullet Dictionary \in k \times n$    
    \For{$i=1$; $i<n$; $i++$ }%
    	\State Get the maximum column value and ID for each row	$\in Projection$
    	 \ForAll { $p\in image\ patches$}
    	 	\State $V_p \in$ $\max$ \{ $basis\ w.r.t.\ image\ patch$ \}
    	 	\State $Reduction(:,p)=V_p\bullet Dictionary( : , ID_{max}(p))$
         \EndFor
         \State $Data=Data - Reduction^T$
         \State $Projection = Data\bullet Dictionary$
    \EndFor 
...
This is the main step of our proposed method. Instead of determining two images' similarity by directly computing the difference of their sparse code, we use the \emph{projection} of image onto dictionary basis as an emulation of image-triggered neuron excitation. We proposed two neural-inspired procedures that when used cooperatively, can generate each image's hash value. For each image, this step can efficiently select from the dictionary a group of (non-redundant) features that together captures the image's characteristic. ``Which features are chosen from the static dictionary'' can be regarded as a form of hash value. If our method can effectively select features in a way just like how we recognizing similarity, the cognitively similar images would have similar hash values, i.e. have similar features selected.

The two neural-inspired procedures: \emph{inhibitive feature selection}, and \emph{synaptic plasticity reactivation scoring} focus on the mutual inhibition of neurons and the connection weight tuning based on experience, respectively. As will be shown in the experiment, the two mechanisms reinforce each other and leads to much better result when used together.

\subsubsection{Inhibitive Feature Selection Algorithm}
-Sparsity: since a vector that best captures one feature is not necessarily independent from vectors that best capture other features
...
\caption{Precision \& Recall curve with 10 random queries for each different category. In each case, the set of images are top 8 retrieved results given a certain query image. Our proposed algorithm outperforms the baselines in different categories }
\end{figure*}
