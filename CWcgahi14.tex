...
Dictionary as an overcomplete basis comes as a natural result under sparsity constraints. Given a training set of $n$ input images $\overrightarrow{i^{(1)}},\overrightarrow{i^{(2)}},...,\overrightarrow{i^{(n)}}$, assume that their sparse coefficients given the to-be-learned basis are $\overrightarrow{c^{(1)}},\overrightarrow{c^{(2)}},...,\overrightarrow{c^{(n)}}$, the basis $\overrightarrow{b^{(1)}},\overrightarrow{b^{(2)}},...,\overrightarrow{c^{(m)}}$ that form the learned dictionary is the solution to the optimization problem:
\[\mbox{minimize}\sum_{i=1}^{m}\]
