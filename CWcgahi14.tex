...
\caption{Precision\& Recall curves with 10 random queries for each different category. In each case, the set of images is an example top 8 retrieved results from one of the 10 random queries. Our proposed method (IFS\_ SPRS) outperforms the baselines in different image categories. Categories from left to right: bamboo forest, beach, corridor.}
\end{figure*}
...
Under our sparse coding dictionary framework, we implemented two encoding method: one is a direct transplant of localitive sensitive hashing method: reducing the high dimensional data into lower dimension by projecting onto the dictionary without considering inhibitions between similar features, although the reactivation scoring is included. This is our first proof-of-concept method to explore the effectiveness of image retrieval under such a sparsity dictionary framework and we will refer to it as \emph{feature selection algorithm}, denoted as \emph{FS} in the figures. Another one is our novel \emph{inhibitive feature selection algorithm}, denoted as \emph{IFS\_SPRS} in the figures. We largely improved the original performance in FS\_SPRS and we can see the outperformance in the Fig 5.
