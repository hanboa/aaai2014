...
Unfortunately, though there are works (Ge et al. 2013) that tackle the similar image search problem by first representing query images as their sparse codes, the high computational cost involved in sparse code calculation renders it infeasible for (near) realtime retrieval tasks. We proposed a novel approach to overcome the performance bottleneck by a change of perspective: instead of encoding the images 
