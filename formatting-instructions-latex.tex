...
\begin{abstract}
\begin{quote}
Despite growing interest in using sparse coding based methods for image classification and retrieval tasks, progress in this direction has been limited by the high computational cost for generating each image's sparse representation, especially in time-critical applications. To tackle this problem, we propose a novel, unsupervised way to efficiently select important high-level features of a query image that determine to which group we would visually perceived as similar. The preliminary results show that our method is indeed suitable for real-time similar image retrieval without much compromise in accuracy. Finally, we consider a more general problem of how to make the pre-learned dictionary to adaptively refine the features contained according to past queries while retaining the efficiency requirement.
